...
You need **one LLM auth secret** (pick whichever works for your billing setup) + `GH_GLOBAL`:

```bash
# LLM auth — set EXACTLY ONE of the following three:
gh secret set CLAUDE_CODE_OAUTH_TOKEN  # ← Claude Pro/Max OAuth (free with subscription)
                                       #   Get via `claude setup-token` (opens browser)
# OR
gh secret set ANTHROPIC_API_KEY        # ← Direct Anthropic API (pay-per-token)
                                       #   Get from console.anthropic.com/settings/keys
# OR
gh secret set OPENROUTER_API_KEY       # ← OpenRouter (access to Claude + GPT + Gemini etc.)
                                       #   Get from openrouter.ai/keys
                                       #   The workflow routes this through Claude Code via
                                       #   Anthropic-compatible endpoint at openrouter.ai/api/v1

# Cross-repo PAT for posting to Westworld:
gh secret set GH_GLOBAL                # Classic GitHub PAT with public_repo scope
                                       # Generated by your host account (not your founder account)

# Variables — non-sensitive, readable from workflows
...
**Auth priority order** (if multiple are accidentally set): OpenRouter > Anthropic direct > Claude OAuth. Workflow picks the first one that's non-empty and configures Claude Code accordingly.
